...
<body <?php body_class(); ?>>
    <header class="header">
        ...
        <nav class="second">
        	...
        	<h2 role="heading" aria-level="2" class="main-nav hidden" ><?php _e('Navigation Principale','b');?></h2>
        	<!-- MENU -->
        		<?php 
        			foreach (b_get_menu_items('main-nav') as $navItem) : ?> <!-- Doit retourner un tab-->
						<li class="second__elt first__elt-second"><a href="<?php echo $navItem->url; ?>" title="" class="nav__link nav__link--<?php echo $navItem->icon; ?>"><?php echo $navItem->label; ?></a>
						</li>
        		<?php endforeach; ?>
        	</ul>
        </nav>
